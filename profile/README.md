...
Tested on a home development machine — AMD Ryzen 9, 32GB RAM, consumer NVMe,
default settings throughout. No tuning. No provisioned infrastructure.

| Query | Substrate | Dataset | Result |
|---|---|---|---|
| 5-constraint, correct plan order | Apache Pinot | 8.9M documents | 42ms |
| 5-constraint, trap order (same query) | Apache Pinot | 8.9M documents | timeout |
| 3-constraint intersection | PostgreSQL | 1M documents | 281ms |

The timeout result is the point. The same query in the wrong dimension order
did not degrade — it failed to return at all. Portolan's selectivity-ordered
planning makes this class of failure structurally impossible.

Your numbers on real hardware will be better. The ratios are structural.
...
Portolan selects correct dimension ordering automatically from live cardinality
probes. A user cannot accidentally produce the timeout by expressing constraints
in the wrong order — Portolan ignores input order and derives execution order
from the data.
